  ...
  return (
    <div className="mb-2 pb-2 border-b border-gray-400 last:border-b-0">
      <div className="flex justify-between items-baseline mb-1">
        <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold text-gray-900">{name}</h3>

        {!isSandwichType && price && <span className="text-xl sm:text-2xl md:text-3xl font-semibold text-gray-900">{price}</span>}
        {!isSandwichType && prices && (
          <div className="flex flex-wrap justify-end gap-x-4 text-xl sm:text-2xl md:text-3xl font-semibold text-gray-900">
            {prices.map((p, index) => (
      ...
      {isSandwichType && (
        <div className="flex justify-end text-gray-900 mt-1">
          ...
          {comboPrice && <span className="w-1/4 text-right text-sm sm:text-base md:text-xl font-bold">{comboPrice}</span>}
          {sandwichPrice && <span className="w-1/4 text-right text-sm sm:text-base md:text-xl font-bold">{sandwichPrice}</span>}
        </div>
      )}
    </div>
  );
